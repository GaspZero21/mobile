...
class _LeaderboardScreenState extends State<LeaderboardScreen>
  ...
  // FIX: use lowercase values 'donors' / 'beneficiaries' as per Swagger docs
  String _typeFilter = '';
  String _cityFilter = '';
  ...
  // FIX: donationCount is a direct field per Swagger schema
  int _extractDonationCount(Map<String, dynamic> user) {
    // 1. Direct field 'donationCount' (Swagger confirmed)
    final v = user['donationCount'];
    ...
    // 2. Fallback aliases
    for (final key in const [
      'donations_count', 'totalDonations', 'total_donations',
      'donationsCount', 'donation_count',
    ]) {
      final val = user[key];
  ...
  Future<void> _fetchLeaderboard() async {
    ...
    try {
      ...
      // FIX: pass lowercase filter values ('donors' / 'beneficiaries')
      ...
      debugPrint('[Leaderboard] body  : ${res.body}');
      ...
      debugPrint('[Leaderboard] items: ${leaders.length}');
  ...
  Future<void> _fetchBadgesById(String id) async {
    try {
      ...
      debugPrint('[Badges] body  : ${res.body}');
      ...
      debugPrint('[Badges] items: ${badges.length}');
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              controller: _tabCtrl,
              children: [_buildRankingsTab(), _buildBadgesTab()],
            ),
          ),
        ],
      ),
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            const Spacer(),
            Image.asset('assets/images/logo_full.png', width: 36, height: 36,
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                ...
                // FIX: lowercase values matching Swagger available values
                _filterChip('🌱 Donors', _typeFilter == 'donors', () {
                  setState(() => _typeFilter = 'donors');
                  _fetchLeaderboard();
                }),
                const SizedBox(width: 8),
                _filterChip('🤲 Beneficiaries', _typeFilter == 'beneficiaries', () {
                  setState(() => _typeFilter = 'beneficiaries');
                  _fetchLeaderboard();
                }),
                const SizedBox(width: 12),
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _cityFilter.isNotEmpty
                          ? kTerra : Colors.white.withOpacity(0.15),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(color: Colors.white.withOpacity(0.3)),
                    ),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      const Icon(Icons.location_on_outlined, color: kWhite, size: 13),
    ...
    if (_leadersError != null) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text(_leadersError!, textAlign: TextAlign.center,
    ...
    if (_leaders.isEmpty) {
      return const Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          SizedBox(height: 12),
          Text('No rankings yet', style: TextStyle(color: kSage, fontSize: 14)),
        ]),
      );
    }
  ...
  Widget _buildLeaderCard(Map<String, dynamic> user, int rank) {
    ...
    // FIX: donationCount is a direct top-level field per Swagger
    final donationCount = _extractDonationCount(user);

    final isTop3     = rank <= 3;
    final medal      = rank == 1 ? '🥇' : rank == 2 ? '🥈' : rank == 3 ? '🥉' : '';
    final cardColor  = rank == 1 ? const Color(0xFFFFF8E1)
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: isTop3
            ? [BoxShadow(color: rankColor.withOpacity(0.2),
        ...
        child: Row(children: [
            ...
            child: ClipOval(
              child: avatar != null && avatar.isNotEmpty
                  ? Image.network(avatar, fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) => _initialsAvatar(name, 44))
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              // FIX: show donationCount from API
              Text('$donationCount donation${donationCount == 1 ? '' : 's'}',
    ...
    if (_badgesError != null) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          const Icon(Icons.wifi_off, color: kSage, size: 36),
          const SizedBox(height: 8),
          Text(_badgesError!, textAlign: TextAlign.center,
              style: const TextStyle(color: kSage, fontSize: 13)),
          TextButton(onPressed: _fetchMyBadges,
              child: const Text('Retry', style: TextStyle(color: kTeal))),
        ]),
      );
    }
      ...
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const Text('No badges yet',
              style: TextStyle(color: kSage, fontSize: 16, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
