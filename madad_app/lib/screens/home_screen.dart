...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // Oran, Algeria — correct coordinates
  ...
  List<dynamic>? _pickList(Map<String, dynamic> map, List<String> keys) {
    dynamic cur = map;
    for (final k in keys) {
      if (cur is Map && cur.containsKey(k)) cur = cur[k];
      else return null;
    }
  ...
  Widget _buildQuantityBadge(Map<String, dynamic> d) {
    ...
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Flexible(
          child: Text(label,
              style: TextStyle(fontSize: 9, color: pillColor,
        ...
        children: [
          ..._categories.map((cat) {
            ...
            return Padding(
              ...
              child: GestureDetector(
                onTap: () {
                  setState(() { _activeCategory = cat['value']!; _isLoading = true; });
                  _fetchDonations();
                },
          ...
          GestureDetector(
            onTap: () {
              setState(() { _filterUrgent = _filterUrgent == true ? null : true; _isLoading = true; });
              _fetchDonations();
            },
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 180),
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(20),
                border: Border.all(
                    color: _filterUrgent == true ? Colors.red : const Color(0xFFDDDDDD)),
              ),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.bolt, size: 12,
                    color: _filterUrgent == true ? kWhite : Colors.orange),
                const SizedBox(width: 4),
                Text('Urgent',
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500,
                        color: _filterUrgent == true ? kWhite : kSage)),
              ]),
            ),
          ),
        ],
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Stack(children: [
              SizedBox(height: 200, width: double.infinity,
                  child: photo != null && photo.isNotEmpty
                      ? Image.network(
                          photo.startsWith('http') ? photo : '$_imgBase$photo',
                          fit: BoxFit.cover,
              ...
              Positioned(top: 10, left: 10,
                  child: GestureDetector(
                    ...
                    child: Container(width: 32, height: 32,
                        decoration: BoxDecoration(
                            color: kWhite.withOpacity(0.9), shape: BoxShape.circle),
              ...
              if (urgent)
                Positioned(bottom: 0, left: 0, right: 0,
                    child: Container(
                      ...
                      child: const Text('⚡ URGENT — Pick up as soon as possible!',
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(title,
                    style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: kTeal)),
                if (category.isNotEmpty) ...[
                  const SizedBox(height: 4),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3),
                    decoration: BoxDecoration(
                        color: kTeal.withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
                    child: Text(category,
                ...
                _infoRow(Icons.circle, 'Status', status,
                    valueColor: status == 'available' ? Colors.green : kTerra),
                if (description.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 4),
                  Text(description,
                      style: const TextStyle(fontSize: 12, color: kSage, height: 1.5)),
                ],
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () { Navigator.pop(context); _showConfirmModal(d); },
                    child: const Text('Reserve This Donation',
  ...
  void _showConfirmModal(Map<String, dynamic> d) {
    ...
    final remainingNum = remaining != null
        ...
        : (totalQty != null
            ...
            : (totalQtyStr.isNotEmpty ? _parseQtyNum(totalQtyStr) : null));
        ...
        child: SingleChildScrollView(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Center(child: Text('Confirm Your Reservation',
                    style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: kTeal))),
                const SizedBox(height: 14),
                ...
                if (pickupType.isNotEmpty) _detailRow('Pickup Type:', pickupType),
                ...
                if (totalQtyStr.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(children: [
                        const Icon(Icons.inventory_2_outlined, size: 13, color: kTeal),
                      ...
                      if (remaining != null) ...[
                        ...
                        Row(children: [
                          ...
                          const SizedBox(width: 6),
                          Text('Still available: ${_fmtNum(remaining)}',
                      ...
                      if (canSplit) ...[
                        const SizedBox(height: 4),
                        const Text('💡 You can reserve a part of this donation',
                  ...
                  children: [
                    TextButton(
                      ...
                      child: const Text('Cancel',
                          style: TextStyle(color: kTerra, fontSize: 14, fontWeight: FontWeight.w600)),
                    ),
                    Row(children: [
                      if (canSplit) ...[
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            ...
                            final qty = await showQuantityPickerSheet(context,
                                totalQuantity: totalQtyStr, remainingQty: remainingNum);
                            if (qty != null && mounted) await _doReserve(d, quantity: qty);
                          },
                      ...
                      ElevatedButton.icon(
                        style: ElevatedButton.styleFrom(
                            ...
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20))),
                        onPressed: () async { Navigator.pop(dialogCtx); await _doReserve(d); },
                        icon: const Icon(Icons.shopping_basket_outlined, color: kWhite, size: 18),
                        label: Text(canSplit ? 'Reserve All' : 'Confirm',
  ...
  Future<void> _doReserve(Map<String, dynamic> d, {double? quantity}) async {
    ...
    if (donationId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Could not identify donation'), backgroundColor: Colors.red));
      return;
    }
    showDialog(context: context, barrierDismissible: false,
        builder: (_) => const Center(child: CircularProgressIndicator(color: kTeal)));
    try {
      await ReservationService().createReservation(donationId, quantity: quantity);
      if (!mounted) return;
  ...
  Future<void> _openAddDonation() async {
    final result = await Navigator.push<bool>(
        context, MaterialPageRoute(builder: (_) => const AddDonationScreen()));
    if (result == true) _fetchAll();
  }

  Widget _photoPlaceholder() => Container(color: kSage,
      child: const Center(child: Icon(Icons.fastfood, color: kWhite, size: 48)));
    ...
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Text(value,
            style: TextStyle(fontSize: 12, color: valueColor ?? kSage,
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // ── LOGO — centred, using the real asset ──────────────
                    Center(
                      child: MadadLogo(
                        height: 70,
                        // White tint so the logo pops on the teal background
                        color: Colors.white,
                      ),
                    ),
                    const SizedBox(height: 14),
                        ...
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Row(children: [
                                _legendDot(Colors.green, 'Fresh'),
                                const SizedBox(width: 8),
                                _legendDot(Colors.orange, 'Dry'),
                                const SizedBox(width: 8),
                        ...
                        ClipRRect(
                          ...
                          child: SizedBox(
                            ...
                            child: FlutterMap(
                              ...
                              options: MapOptions(initialCenter: _oranCenter, initialZoom: 13),
                              children: [
                                TileLayer(
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Expanded(
                            child: Text(
                              _activeCategory.isEmpty && _filterUrgent == null
                                  ? 'Nearby Donations'
                                  : 'Filtered Donations',
                          ...
                          GestureDetector(
                            ...
                            child: Row(children: const [
                              Text('View All',
                                  style: TextStyle(fontSize: 13, color: kTerra,
                                      fontWeight: FontWeight.w600)),
                              SizedBox(width: 2),
                      ...
                      child: _isLoading
                          ...
                          : _error != null
                              ? Center(child: Column(
                                  ...
                                  children: [
                                    const Icon(Icons.wifi_off, color: kSage, size: 32),
                                    const SizedBox(height: 8),
                                    const Text('Could not load donations',
                                        style: TextStyle(color: kSage, fontSize: 13)),
                                    TextButton(onPressed: _fetchAll,
                              ...
                              : _donations.isEmpty
                                  ? Center(child: Column(
                                      mainAxisAlignment: MainAxisAlignment.center,
                                      children: [
                                        const Icon(Icons.inbox_outlined, color: kSage, size: 36),
                                        const SizedBox(height: 8),
                                        Text(
                                          _activeCategory.isNotEmpty || _filterUrgent != null
                                              ? 'No donations match this filter'
                                              : 'No donations yet.\nBe the first to post!',
                                          textAlign: TextAlign.center,
                                          style: const TextStyle(color: kSage, fontSize: 13),
                                        ),
                                        if (_activeCategory.isEmpty && _filterUrgent == null)
                                          TextButton(onPressed: _openAddDonation,
                      ...
                      child: Container(
                        ...
                        child: _statsLoading
                            ...
                            : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                _buildStat('Donations', _donationCount, kTeal,
                                    (_donationCount / 10.0).clamp(0.0, 1.0)),
                                const SizedBox(height: 14),
                                _buildStat('Reservations', _requestCount, kTerra,
                                    (_requestCount / 10.0).clamp(0.0, 1.0)),
                                const SizedBox(height: 14),
                                _buildStat('Food Saved', _foodSavedKg > 0
                                    ? double.parse(_foodSavedKg.toStringAsFixed(1)) : 0,
                                    kSage, (_foodSavedKg / 50.0).clamp(0.0, 1.0), suffix: ' '),
                                const SizedBox(height: 14),
                                _buildReputation(),
                              ]),
                      ),
                ...
                child: photo != null && photo.isNotEmpty
                    ? Image.network(
                        ...
                        errorBuilder: (_, __, ___) => Container(
                            color: kSage, child: const Icon(Icons.fastfood, color: kWhite, size: 32)))
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                if (urgent)
                  Container(
                    ...
                    child: const Text('!',
                        style: TextStyle(fontSize: 9, color: kWhite, fontWeight: FontWeight.bold)),
                  ),
              ]),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                      color: kTerra, borderRadius: BorderRadius.circular(20)),
                  child: const Center(
                    child: Text('Reserve',
                        style: TextStyle(fontSize: 10, color: kWhite, fontWeight: FontWeight.bold)),
                  ),
                ),
              ),
            ]),
  ...
  Widget _legendDot(Color color, String label) => Row(children: [
    ...
    const SizedBox(width: 3),
    Text(label, style: const TextStyle(fontSize: 9, color: kSage)),
  ]);
    ...
    return Row(children: [
      ...
      Expanded(child: ClipRRect(
        ...
        child: LinearProgressIndicator(value: ratio,
            ...
            valueColor: AlwaysStoppedAnimation<Color>(color), minHeight: 9),
      )),
  ...
  Widget _buildReputation() {
    ...
    return Row(children: [
      const SizedBox(width: 90,
          child: Text('Reputation', style: TextStyle(fontSize: 13, color: Colors.black87))),
      Row(children: List.generate(5, (i) {
        final v = i + 1;
        return Icon(filled >= v ? Icons.star : filled >= v - 0.5 ? Icons.star_half : Icons.star_border,
            color: Colors.amber, size: 20);
      })),
      const SizedBox(width: 8),
      Text(_reputation > 0 ? _reputation.toStringAsFixed(1) : '—',
